...
|---|---|---|
| `gmail.send` | **Sensitive** | Yes — the only Gmail scope V1 requests |
| `openid`, `email` | Non-sensitive | Yes — identity only, so the callback can read the connected address from OIDC userinfo (`gmail.send` cannot call `users.getProfile`) |
| `gmail.compose` | **Restricted** | No — deferred with D1 |
| `gmail.readonly` | **Restricted** | No — deferred with D1 |

Initial connect requests **`openid email gmail.send`** — `gmail.send` is the only Gmail scope; the two identity scopes let the callback read the connected address from Google's OIDC userinfo endpoint, since a `gmail.send`-only token is refused by Gmail's `users.getProfile`. Requesting *any* Restricted scope at connect time puts the entire Gmail integration behind a CASA security assessment — not just the actions that use it — so `gmail.compose` was removed from the initial tier (A15). The "Enable email reading" flow adds the restricted scopes with `include_granted_scopes=true` and belongs to the deferred D1 program; `gmail_connections.scopes` stores what Google actually granted.
